    ...
    def __call__(self, noisy_data):
        ...
        if self.mode == 'eigenvalues':
            ...
            try:
                ...
                # Handle numerical instability: compute eigenvalues/eigenvectors element by element
                eigvals_list = []
                eigvectors_list = []
                for i in range(bs):
                    try:
                        eigvals_i, eigvectors_i = torch.linalg.eigh(L[i])
                        eigvals_list.append(eigvals_i)
                        eigvectors_list.append(eigvectors_i)
                    except RuntimeError:
                        # Fallback: use identity for failed cases
                        eigvals_list.append(torch.zeros(n, device=L.device, dtype=L.dtype))
                        eigvectors_list.append(torch.eye(n, device=L.device, dtype=L.dtype))
                eigvals = torch.stack(eigvals_list, dim=0)
                eigvectors = torch.stack(eigvectors_list, dim=0)
...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    random = torch.randn(bs, n, device=vectors.device) * (~node_mask)                                   # bs, n
