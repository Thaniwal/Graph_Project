...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def forward(self, noisy_data, extra_data, node_mask):
        ...
        # Ensure both have batch as first dimension for concatenation
        if y_t.dim() == 1:
            y_t = y_t.unsqueeze(-1)
        if extra_y.dim() == 1:
            extra_y = extra_y.unsqueeze(-1)
        ...
        if y.shape[-1] != expected_y_dim:
            # Slice or pad to match expected dimension
            if y.shape[-1] > expected_y_dim:
                y = y[:, :expected_y_dim]
            else:
                # Pad if too small (unlikely but handle it)
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        p_s_and_t_given_0_E = diffusion_utils.compute_batched_over0_posterior_distribution(X_t=E_t,
                                                                                           ...
                                                                                           Qsb=Qsb.E,
                                                                                           Qtb=Qtb.E)
        # Ensure posterior distributions are on the same device as predictions
