...
def get_resume(cfg, model_kwargs):
    ...
    OmegaConf.set_struct(new_cfg.general, False)
    new_cfg.general.test_only = resume
    new_cfg.general.name = name
    OmegaConf.set_struct(new_cfg.general, True)  # Re-enable struct mode
...
def main(cfg: DictConfig):
    ...
    if ('test_only' in cfg.general) and cfg.general.test_only:
        ...
        from omegaconf import OmegaConf
        test_only_path = OmegaConf.select(cfg, 'general.test_only', default=None)
        if test_only_path:
            os.chdir(test_only_path.split('checkpoints')[0])
